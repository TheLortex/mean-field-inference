...
class MeanField():
    ...
    def build_model (self, weights, unary, n_iter, damping=0.05, FNN=(0,0,0,1)), placeholder=tf.placeholder(tf.float32, shape=[None, 2, self._n, self._m, self._p], name="theta_clip"):
...
class TensorflowizedBatchedMultiModalMeanField():
    ...
    def find_phase_transition(self, currentModes, nmodes, T):
        ...
        entropy0 = (-tf.reduce_sum(q0*tf.log(q+0.0000001),axis=4)) < 0.3*tf.log(tf.constant(10))


        results_q = tf.zeros((self._n, self._n, self._p, self._bs, nmodes))
        unfinished = tf.convert_to_tensor(np.array([[True]*self._nmodes for _ in range(self._bs)]), dtype=tf.bool)
        ...
        def body(unfinished, n_iter, T, results_q):
            ...
            phase_transition = tf.logical_and(entropy0, tf.greater(entropyi, 0.7*tf.log(tf.constant(10)))) # of shape bs,nmodes,n,n
            ...
            should_update_n_n_p = tf.expand(dims(should_update_n_n,-1)


            n_iter_next = tf.cond(tf.equal(tf.reduce_sum(should_update), 0), lambda: tf.add(n_iter, 1), lambda: n_iter)
            T_next = T * 1.2
            unfinished_next = tf.logical_xor(should_update, unfinished)
            results_q_next = tf.where(should_update_n_n_p, q, results_q)
            
            return unfinished_next, n_iter_next, T_next, results_q_next

        _,_,_,q_final = tf.while(cond,body,(unfinished,n_iter,T,results_q)
        return q_final

    def iteration(self, currentModes, nmodes): 
        q = self.find_phase_transition(currentModes, nmodes, tf.constant(1/5.))
        entropy = tf.reduce_sum(q*log(q+0.0000001), axis=4)
        ...
        q_reshaped  = tf.reshape(q_selected,[self._bs*nmodes, self._bs*nmodes, p])
        ...
        q_maxentropy = tf.reshape(q_flat_maxentropy, [self._bs,nmodes,p]) # of shape (bs, nmodes)
        maxq_maxentropy = tf.argmax(q_maxentropy, axis=2) # of shape (bs, nmodes)

        bs_list = tf.expand_dims(tf.cumsum(tf.ones([self._bs]), exclusive=True),1)
        modes_list = tf.expand_dims(tf.cumsum(tf.ones([nmodes]), exclusive=True),0)
        ...
        n0_max_entropy_pos = tf.div(max_entropy_pos, self._n)
        n1_max_entropy_pos = tf.mod(max_entropy_pos, self._n)


        update_coordinates_0 = tf.stack([bs_mtrx, modes_mtrx, tf.zeros([self._bs,nmodes],dtype=tf.int64),n0, n1, maxq_maxentropy], axis=2) # of shape (bs, nmodes, 6)
        update_coordinates_1 = tf.stack([bs_mtrx, modes_mtrx, tf.ones([self._bs,nmodes],dtype=tf.int64),n0, n1, maxq_maxentropy], axis=2) # of shape (bs, nmodes, 6)


        ref = currentModes # of shape (bs, nmodes, 2, n, n, p), P = 6
        indices = update_coordinates_0 # of shape (bs, nmodes, 6) Q = 3
        # K = P
        # updates of rank Q-1, shape (bs, nmodes)
        updates = 100*tf.ones([self._bs, nmodes])
        
        modesClampPlus = tf.scatter_nd_add(currentModes,update_coordinates_0,updates)
        # set cur_mode[0,x,y,k] += 100 => -50 to 50: 
        modesClampMinus = tf.scatter_nd_sub(currentModes,update_coordinates_0,updates)
